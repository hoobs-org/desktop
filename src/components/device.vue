<template>
    <div id="device">
        ...
        <div v-if="joined" class="action-cell">
            <confirm v-if="show.restart" value="Restart Bridge" icon="cached" title="Are you sure you want to restart the bridge?" v-on:start="toggleFields(false, true, false, false)" v-on:cancel="toggleFields(true, true, true, true)" v-on:confirm="restartDevice()" />
        </div>
        <div v-if="joined" class="action-cell">
            <div v-if="show.terminal" v-on:click="$emit('terminal')" title="Terminal" class="icon action-icon">code</div>
        </div>
        <div v-if="joined" class="action-cell">
            <confirm v-if="show.reboot" value="Reboot Device" icon="power_settings_new" title="Are you sure you want to restart this device?" v-on:start="toggleFields(false, false, true, false)" v-on:cancel="toggleFields(true, true, true, true)" v-on:confirm="rebootDevice()" />
        </div>
        <div v-if="joined" class="action-cell">
            <confirm v-if="show.remove" value="Remove Device" icon="delete_outline" title="Are you sure you want to remove this device?" v-on:start="toggleFields(false, false, false, true)" v-on:cancel="toggleFields(true, true, true, true)" v-on:confirm="$emit('remove')" />
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            toggleFields(terminal, restart, reboot, remove) {
            ...
            async restartDevice() {
                this.toggleFields(true, false, false, true);
                this.show.working = true;

                await this.api.post(this.value.ip, this.value.port, "/service/restart");

                this.show.working = false;
                this.toggleFields(true, true, true, true);
            },

            async rebootDevice() {
                this.toggleFields(false, false, false, true);
                this.show.working = true;
