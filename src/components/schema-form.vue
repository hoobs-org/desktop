    ...
    <div id="schema-form">
        <div v-for="(field, index) in fields" :key="index">
            <div v-if="fieldType(field) === 'input'">
                <component v-on:input="markDirty()" :is="getComponent(field)" :name="field.title || humanize(field.name)" :options="getOptions(field)" :type="getType(field)" :required="field.required" :description="field.description || ''" v-model="value[field.name]" />
            </div>
            <div v-else-if="fieldType(field) === 'form'">
                <schema-form v-on:input="markDirty()" :schema="field.properties" v-model="value[field.name]" />
            </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            fields() {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    this.defineDefaults(field.name, field);
        ...
        mounted() {
            ...
            for (let i = 0; i < keys.length; i++) {
                this.defineDefaults(keys[i], this.schema[keys[i]]);
            }
        ...
        methods: {
            ...
            defineDefaults(name, field) {
                if (field.default !== undefined && this.value[name] === undefined) {
                    this.value[name] = field.default;
                }

                if (field.const !== undefined) {
                    this.value[name] = field.const;
                }
            },
